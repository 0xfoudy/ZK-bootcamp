...
contract Pairings {
     ...
     */
     function run(uint256[12] memory input) public view returns (bool success) {
        (success, ) = address(8).staticcall(abi.encode(input));
        if (!success) revert("Wrong pairing");
    ...
    function run2(bytes calldata input) public view returns (bool) {
        ...
        if (input.length % 192 != 0) revert("Points must be a multiple of 6"); // 6 * 32
